...
public class PointLogRepositoryImpl implements PointLogRepositoryCustom {
  ...
  public Page<PointLog> search(PointLogSearchRequest request) {
    ...
    if (request.userId() != null) {
      filter.append(
          " AND p.user.id = :userId ");//p (PointLog) → user (User 객체) → id (User 객체의 ID) 순서대로 접근
    }
    ...
    String jpql =
        baseQuery + filter + " ORDER BY p." + request.sortBy() + " " + request.direction();
    var query = em.createQuery(jpql,
        PointLog.class);//PointLog.class는 TypedQuery<PointLog>를 반환 한다고 알려줌
